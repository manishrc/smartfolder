...
import { FolderConfig, ToolId } from '../config';
    ...
    async invoke(args, ctx) {
      ...
      if (fromExt && toExt && fromExt !== toExt) {
        return errorResult(
          ...
          `${fromArg} -> ${toArg}`,
          new Error(
            `File extension must be preserved. Original file has extension '${fromExt}' but new name has '${toExt}'. Use '${path.basename(
              toArg,
              toExt
            )}${fromExt}' instead.`
          )
        ...
        if (isFile) {
          ...
          if (fromExt && toExt && fromExt !== toExt) {
            return errorResult(
              ...
              `${fromArg} -> ${toArg}`,
              new Error(
                `File extension must be preserved. Original file has extension '${fromExt}' but destination has '${toExt}'. Use '${path.basename(
                  toArg,
                  toExt
                )}${fromExt}' instead.`
              )
