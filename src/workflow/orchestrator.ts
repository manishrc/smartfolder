...
> {
      ...
      if (totalLines > HEAD_LINES + TAIL_LINES) {
        ...
        previewLines.push(
          `... (${totalLines -
            HEAD_LINES -
            TAIL_LINES -
            (headerLine ? 1 : 0)} lines omitted) ...`
        );
        previewLines.push('');
        previewLines.push(...tailLines);

        fileContentPreview = `\n\nFile content preview (truncated - showing first ${HEAD_LINES} and last ${TAIL_LINES} lines${
          headerLine ? ', header preserved' : ''
        }):\n\n\`\`\`\n${previewLines.join('\n')}\n\`\`\``;
      } else {
  ...
  }.${metadataInfo}${fileContentPreview}

IMPORTANT: If you need to change this file's name, use rename_file with 'from'="${
    event.relativePath
  }" and 'to'="<new-name>${originalExtension}". You MUST preserve the original file extension (${originalExtension}). 
...
CRITICAL: Do NOT use write_file - that creates a new file and leaves the original untouched. Only use write_file if the prompt explicitly asks you to create a new file (like a summary or report). For renaming, ALWAYS use rename_file.`;
  ...
  if (event.mimeType && isBinaryFile(event.mimeType)) {
    try {
      ...
      return `${textPart} (Unable to read file as binary: ${
        (error as Error).message
      })`;
    }
  }
